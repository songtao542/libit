...
public class SwipeBackLayout extends ViewGroup {
    ...
    public SwipeBackLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        setOnSwipeBackListener(new OnSwipeBackListener() {
            @Override
    ...
    public View findAllScrollViews(ViewGroup mViewGroup) {
    ...
    public boolean isScrollableView(View mView) {
    ...
    public boolean contains(View mView, float x, float y) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (innerScrollView != null && contains(innerScrollView, downX, downY)) {
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        ...
        public int clampViewPositionHorizontal(@NonNull View child, int left, int dx) {
            ...
            if (isSwipeEnabled()) {
                if (mDirectionMode == FROM_LEFT && !canViewScrollRight(innerScrollView, downX, downY, false)) {
                    leftOffset = Math.min(Math.max(left, getPaddingLeft()), width);
                } else if (mDirectionMode == FROM_RIGHT && !canViewScrollLeft(innerScrollView, downX, downY, false)) {
        ...
        public int clampViewPositionVertical(@NonNull View child, int top, int dy) {
            ...
            if (isSwipeEnabled()) {
                if (mDirectionMode == FROM_TOP && !canViewScrollUp(innerScrollView, downX, downY, false)) {
                    topOffset = Math.min(Math.max(top, getPaddingTop()), height);
                } else if (mDirectionMode == FROM_BOTTOM && !canViewScrollDown(innerScrollView, downX, downY, false)) {
        ...
        public boolean canViewScrollUp(View mView, float x, float y, boolean defaultValueForNull) {
        ...
        public boolean canViewScrollDown(View mView, float x, float y, boolean defaultValueForNull) {
            if (mView == null || !contains(mView, x, y)) {
                return defaultValueForNull;
            }
            return mView.canScrollVertically(1);
        ...
        public boolean canViewScrollRight(View mView, float x, float y, boolean defaultValueForNull) {
            if (mView == null || !contains(mView, x, y)) {
                return defaultValueForNull;
            }
        ...
        public boolean canViewScrollLeft(View mView, float x, float y, boolean defaultValueForNull) {
            if (mView == null || !contains(mView, x, y)) {
                return defaultValueForNull;
            }
            return mView.canScrollHorizontally(1);
    ...
    public void enableSwipeBack(boolean enable) {
        this.isSwipeEnabled = enable;
    }

    private OnSwipeBackListener mSwipeBackListener;

    public void setOnSwipeBackListener(OnSwipeBackListener mSwipeBackListener) {
