...
open class PhotoViewerFragment : Fragment(), PhotoFragment.OnPhotoSingleTapListener, KeyEventListener {
    ...
    private val mBackInterpolator = PathInterpolator(0f, 0.4f, 0.6f, 1f)
    private val mDeleteInterpolator = PathInterpolator(1f, 0f, 1f, 0f)
    ...
    internal fun update(alpha: Int) {
        val factor = mBackInterpolator.getInterpolation(alpha / 255f)
        val fAlpha = (factor * 255f).toInt()
        view?.setBackgroundColor(Color.argb(fAlpha, 0, 0, 0))
        mDelete.alpha = mDeleteInterpolator.getInterpolation(alpha / 255f)
    }
