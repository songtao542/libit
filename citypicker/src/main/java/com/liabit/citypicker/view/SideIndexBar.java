package com.liabit.citypicker.view;
...
public class SideIndexBar extends View {
    ...
    public void enableHotCities(boolean enableHotCities) {
        if (enableHotCities) {
            if (!mIndexItems.contains(DEFAULT_INDEX_ITEMS[1])) {
                if (!mIndexItems.contains(DEFAULT_INDEX_ITEMS[0])) {
                    mIndexItems.add(0, DEFAULT_INDEX_ITEMS[1]);
                } else {
                    mIndexItems.add(1, DEFAULT_INDEX_ITEMS[1]);
                }
            }
        } else {
            mIndexItems.remove(DEFAULT_INDEX_ITEMS[1]);
        }
        postInvalidate();
    }
    ...
    public void enableLocation(boolean enableLocation) {
        if (enableLocation) {
            if (!mIndexItems.contains(DEFAULT_INDEX_ITEMS[0])) {
                mIndexItems.add(0, DEFAULT_INDEX_ITEMS[0]);
            }
        } else {
            mIndexItems.remove(DEFAULT_INDEX_ITEMS[0]);
        }
        postInvalidate();
    }
