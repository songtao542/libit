...
/**
 * 从 androidx.customview.widget.ViewDragHelper 拷贝过来，
 * 主要为了实现 上下左右 不同的 EdgeSize 和 捕捉非直接Child {@link #captureChildView}
 *
 * <p>
...
@SuppressWarnings({"unused", "ConstantConditions", "JavaDoc",
        "WeakerAccess", "PointlessNullCheck",
        "SameParameterValue", "SuspiciousNameCombination", "BooleanMethodIsAlwaysInverted"})
public class ViewDragHelper {
    ...
    private int mLeftEdgeSize;
    private int mBottomEdgeSize;
    ...
    public abstract static class Callback {
         ...
         * @param xVelocity     X velocity of the pointer as it left the screen in pixels per second.
         * @param yVelocity     Y velocity of the pointer as it left the screen in pixels per second.
         */
         ...
         * @see #EDGE_BOTTOM
         */
        public void onEdgeDragStarted(int edgeFlags, int pointerId, int x, int y) {
        }
         ...
         * @param child
         */
        public void onSettlingComplete(@NonNull View child, int left, int top) {
        }
    }
    ...
    private ViewDragHelper(@NonNull Context context, @NonNull ViewGroup forParent,
        ...
        mLeftEdgeSize = (int) (EDGE_SIZE * mDensity + 0.5f);
        mRightEdgeSize = mLeftEdgeSize;
        mTopEdgeSize = mLeftEdgeSize;
        mBottomEdgeSize = mLeftEdgeSize;
     ...
     * {@link Callback#onEdgeDragStarted(int, int, int, int)} methods will only be invoked
    ...
    public int getLeftEdgeSize() {
        return mLeftEdgeSize;
    }

    @Px
    public int getRightEdgeSize() {
        return mRightEdgeSize;
    }
    ...
    public int getBottomEdgeSize() {
        return mBottomEdgeSize;
    }

    public void setEdgeSize(int dip) {
        mLeftEdgeSize = (int) (dip * mDensity + 0.5f);
        mRightEdgeSize = mLeftEdgeSize;
        mTopEdgeSize = mLeftEdgeSize;
        mBottomEdgeSize = mLeftEdgeSize;
    }
     ...
     * @param dip
     */
    public void setHorizontalEdgeSize(int dip) {
        mLeftEdgeSize = (int) (dip * mDensity + 0.5f);
        mRightEdgeSize = mLeftEdgeSize;
    }
     ...
     * @param dip
     */
    public void setVerticalEdgeSize(int dip) {
        mTopEdgeSize = (int) (dip * mDensity + 0.5f);
        mBottomEdgeSize = mTopEdgeSize;
    }


    public void setLeftEdgeSize(int dip) {
        mLeftEdgeSize = (int) (dip * mDensity + 0.5f);
    }

    public void setRightEdgeSize(int dip) {
        mRightEdgeSize = (int) (dip * mDensity + 0.5f);
    }

    public void setTopEdgeSize(int dip) {
        mTopEdgeSize = (int) (dip * mDensity + 0.5f);
    }

    public void setBottomEdgeSize(int dip) {
        mBottomEdgeSize = (int) (dip * mDensity + 0.5f);
    }

    public void setLeftEdgePixelSize(int px) {
        mLeftEdgeSize = px;
    }

    public void setRightEdgePixelSize(int px) {
        mRightEdgeSize = px;
    }

    public void setTopEdgePixelSize(int px) {
        mTopEdgeSize = px;
    }

    public void setBottomEdgePixelSize(int px) {
        mBottomEdgeSize = px;
    }
    ...
    private boolean forceSettleCapturedViewAt(int finalLeft, int finalTop, int xvel, int yvel) {
        float startLeft = isTranslationMode ? mCapturedView.getTranslationX() : mCapturedView.getLeft();
        float startTop = isTranslationMode ? mCapturedView.getTranslationY() : mCapturedView.getTop();

        final int dx = (int) (finalLeft - startLeft);
        final int dy = (int) (finalTop - startTop);

        if (dx == 0 && dy == 0) {
            // Nothing to do. Send callbacks, be done.
            mCallback.onSettlingComplete(mCapturedView, finalLeft, finalTop);
            mScroller.abortAnimation();
            setDragState(STATE_IDLE);
            return false;
        }
        ...
        mScroller.startScroll((int) startLeft, (int) startTop, dx, dy, duration);
    ...
    public void flingCapturedView(int minLeft, int minTop, int maxLeft, int maxTop) {
        ...
        float left = isTranslationMode ? mCapturedView.getTranslationX() : mCapturedView.getLeft();
        float top = isTranslationMode ? mCapturedView.getTranslationY() : mCapturedView.getTop();

        mScroller.fling((int) left, (int) top,
                (int) mVelocityTracker.getXVelocity(mActivePointerId),
                (int) mVelocityTracker.getYVelocity(mActivePointerId),
    ...
    public boolean continueSettling(boolean deferCallbacks) {
        if (mDragState == STATE_SETTLING) {
            View capturedView = mCapturedView;
            boolean keepGoing = mScroller.computeScrollOffset();
            final int x = mScroller.getCurrX();
            final int y = mScroller.getCurrY();
            final int dx = isTranslationMode ? (int) (x - capturedView.getTranslationX()) : x - capturedView.getLeft();
            final int dy = isTranslationMode ? (int) (x - capturedView.getTranslationY()) : y - capturedView.getTop();

            if (dx != 0) {
                if (isTranslationMode) {
                    capturedView.setTranslationX(x);
                } else {
            ...
            if (dy != 0) {
                if (isTranslationMode) {
                    capturedView.setTranslationY(y);
                } else {
            ...
            if (keepGoing && x == mScroller.getFinalX() && y == mScroller.getFinalY()) {
                ...
                mScroller.abortAnimation();
                keepGoing = false;
                mCallback.onSettlingComplete(capturedView, x, y);
            }
    ...
    /**
     * Like all callback events this must happen on the UI mBackThread, but release
    ...
    private void dispatchViewReleased(float xvel, float yvel) {
        ...
        //Log.d("ViewDragHelper", "dispatchViewReleased mCapturedView=" + mCapturedView);
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                for (int i = 0; i < pointerCount; i++) {
                    ...
                    reportNewEdgeDrags((int) x, (int) y, dx, dy, pointerId);
                    //Log.d("ViewDragHelper", "mDragState=====>" + mDragState);
                    if (mDragState == STATE_DRAGGING) {
                    ...
                    if (pastSlop && tryCaptureViewForDrag(toCapture, pointerId)) {
                        //Log.d("ViewDragHelper", "tryCaptureViewForDrag=====>true" + "  mDragState=" + mDragState);
                        break;
                    }
                }
                saveLastMotion(ev);
    ...
    public void processTouchEvent(@NonNull MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP: {
                //Log.d("ViewDragHelper", "processTouchEvent ACTION_UP mDragState=" + mDragState);
                if (mDragState == STATE_DRAGGING) {
                    releaseViewForPointerUp();
                }
                cancel();
            ...
            case MotionEvent.ACTION_CANCEL: {
                //Log.d("ViewDragHelper", "processTouchEvent ACTION_CANCEL mDragState=" + mDragState);
                if (mDragState == STATE_DRAGGING) {
    ...
    private void reportNewEdgeDrags(int x, int y, float dx, float dy, int pointerId) {
        int dragsStarted = 0;
        ...
        if (dragsStarted != 0) {
            ...
            mCallback.onEdgeDragStarted(dragsStarted, pointerId, x, y);
        }
    }
    ...
    private void dragTo(int left, int top, int dx, int dy, int pointerId) {
        ...
        if (dx != 0) {
            ...
            if (isTranslationMode) {
                ...
                ViewCompat.offsetLeftAndRight(mCapturedView, (int) (clampedX - oldLeft));
            }
        }
        if (dy != 0) {
            ...
            if (isTranslationMode) {
                ...
                ViewCompat.offsetTopAndBottom(mCapturedView, (int) (clampedY - oldTop));
            }
        }

        if (dx != 0 || dy != 0) {
            float clampedDx = clampedX - oldLeft;
            float clampedDy = clampedY - oldTop;
            mCallback.onViewPositionChanged(mCapturedView, clampedX, clampedY, (int) clampedDx, (int) clampedDy);
        }
    }
    ...
    public View findTopChildUnder(int x, int y) {
        View view = mCallback.getChildAt(x, y);
        if (view != null) {
            return view;
        }
    ...
    private int getEdgesTouched(int x, int y) {
        ...
        if (x < mParentView.getLeft() + mLeftEdgeSize) result |= EDGE_LEFT;
        if (y < mParentView.getTop() + mTopEdgeSize) result |= EDGE_TOP;
        if (x > mParentView.getRight() - mRightEdgeSize) result |= EDGE_RIGHT;
        if (y > mParentView.getBottom() - mBottomEdgeSize) result |= EDGE_BOTTOM;
