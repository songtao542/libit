...
class AlertController {
    ...
    private int mButtonPanelLayoutHint = GreeAlertDialog.LAYOUT_HINT_NONE;

    Handler mHandler;

    private boolean mIsDismissOnClickBtn = true;
    private boolean mIsMessageCenter;

    public void setDismissOnClickBtn(boolean enable) {
        mIsDismissOnClickBtn = enable;
    }
    ...
    public AlertController(Context context, GreeAlertDialog di, Window window) {
        mContext = context;
        mDialog = di;
        mWindow = window;
    ...
    private int selectContentView() {
        ...
        if (mButtonPanelLayoutHint == GreeAlertDialog.LAYOUT_HINT_SIDE) {
    ...
    public void setTitleTextSize(int sizePx) {
        ...
        if (mTitleView != null) {
            mTitleView.setTextSize(TypedValue.COMPLEX_UNIT_PX, sizePx);
        }
    }
    public void setMessageCenter(boolean center) {
        mIsMessageCenter = center;
        if (mMessageView != null && center) {
            mMessageView.setGravity(Gravity.CENTER_HORIZONTAL);
        }
    }

    /**
     * @see GreeAlertDialog.Builder#setCustomTitle(View)
     */
     ...
     * @return the panel to use, Null able
     */
    ...
    private void setScrollIndicators(ViewGroup contentPanel, View content,
        ...
        View indicatorUp = null;//mWindow.findViewById(R.id.scrollIndicatorUp);
        View indicatorDown = null;//mWindow.findViewById(R.id.scrollIndicatorDown);

        if (Build.VERSION.SDK_INT >= 23) {
            // We're on Marshmallow so can rely on the View APIs
         //   content.setScrollIndicators(indicators, mask);    // 删除这个滑动边界指示（一条黑线）
            ...
            if (indicatorUp != null && (indicators & View.SCROLL_INDICATOR_TOP) == 0) {
                contentPanel.removeView(indicatorUp);
                indicatorUp = null;
            }
            if (indicatorDown != null && (indicators & View.SCROLL_INDICATOR_BOTTOM) == 0) {
                contentPanel.removeView(indicatorDown);
                indicatorDown = null;
            }
            ...
            if (indicatorUp != null || indicatorDown != null) {
                ...
                if (mMessage != null) {
                    ...
                    mScrollView.setOnScrollChangeListener(
                            new ScrollView.OnScrollChangeListener() {
                                @Override
    ...
    private void setupButtons(ViewGroup buttonPanel) {
        ...
        if (TextUtils.isEmpty(mButtonNeutralText) && mButtonNeutralIcon == null) {
            ...
            View space = buttonPanel.findViewById(R.id.bottom_bar_spacer);
            if (space != null) {
                space.setVisibility(View.INVISIBLE);
            }
            LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT, 0);
    ...
    public static class AlertParams {
        ...
        public boolean mIsDismissOnClickBtn = true; //原生默认就是自动dismiss
        public int mTitleTextSize = 0;
        public boolean mIsMessageCenter = false;
        ...
        public void apply(AlertController dialog) {
            ...
            dialog.setDismissOnClickBtn(mIsDismissOnClickBtn);
            dialog.setTitleTextSize(mTitleTextSize);
            dialog.setMessageCenter(mIsMessageCenter);
            /*
